...
OPENTRIPMAP_KEY = st.secrets.get("OPENTRIPMAP_KEY")
ORS_API_KEY = st.secrets.get("ORS_API_KEY") 

# Check keys
missing = [k for k, v in {"OpenAI": OPENAI_API_KEY, "OpenTripMap": OPENTRIPMAP_KEY}.items() if not v]
if missing:
    st.error(f"⚠️ Missing API keys in Streamlit Secrets: {', '.join(missing)}")
...
def travel_time_between(lat1, lon1, lat2, lon2, mode="foot"):
    ...
    try:
        r = requests.get(f"http://router.project-osrm.org/route/v1/{profile}/{coords}?overview=false", timeout=8)
...
def build_greedy_itinerary(pois, start_lat, start_lon, days=1, hours_per_day=8, mode="foot"):
    ...
    for p in poi_list:
        travel_min = travel_time_between(current_lat, current_lon, p["_lat"], p["_lon"], mode)
        visit_min = 60
        ...
        if current_day["minutes_left"] >= total_needed:
            ...
            if day > days:
                break
            current_day = {"day": day, "items": [], "minutes_left": hours_per_day*60}
            travel_min = travel_time_between(start_lat, start_lon, p["_lat"], p["_lon"], mode)
...
for key in ["itinerary","lat","lon","llm_text"]:
